...
class ProtoFactory:
    ...
    def get_object_type(self, pid_str: str) -> Optional[Type]:
        ...
        return PROTO_OBJECTS.get(prefix)
    ...
    def from_dict(self, data: dict):
        ...
        if not obj_type or not hasattr(obj_type, "from_dict"):
            raise ValueError(f"Cannot construct object for pid: {pid}")
    ...
    def load(self, path: Union[str, Path]) -> List[object]:
        ...
        return [self.from_dict(entry) for entry in data if "pid" in entry]
...
#             new_obj = obj_type.from_dict(data)
