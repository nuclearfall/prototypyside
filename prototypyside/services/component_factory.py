...
class ComponentFactory:
    ...
    def create(self, pid: str, **kwargs) -> object:
        ...
        **kwargs : dict
        ...
        try:
            # The object's constructor (__init__) must accept 'pid' as a keyword argument
            # and other relevant kwargs for its initialization.
            return obj_type(pid=pid, **kwargs)
        except TypeError as e:
            raise TypeError(f"Failed to instantiate {obj_type.__name__} with PID '{pid}' and kwargs {kwargs}: {e}")
        except Exception as e:
            raise RuntimeError(f"An unexpected error occurred during creation of {obj_type.__name__} with PID '{pid}': {e}")
