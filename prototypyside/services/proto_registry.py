...
#         if template_pid:
#             template = self._objects[template_pid]
...
#         templates = [o for o in self._objects if hasattr(o, "is_template")]
#         # elements = [o for o in self._objects if hasattr(o, "template_pid")]
#         for item in templates:
#             for child_pid in template.element_pids():
#                 print("Child object found. Should be inserted.")
#                 child_obj = self.get(child_pid) 
#                 item.insert_element(child_obj)
...
#         if hasattr(obj, "template_pid"):
#             print("Here an element should be inserted into the template")
#             template = self.get(obj.template_pid)
#             print(template.pid)
#             if template:
#                 template.insert_element(obj)
#                 print(templant.elements)
#         return obj
...
#         self._objects.clear()
#         objs_data = data.get("objects", {})
#         root_pid = data.get("root")
        
#         if root_pid not in objs_data:
#             raise ValueError(f"Root object '{root_pid}' not found in data.")

#         # Step 1: Load and register root object (ComponentTemplate)
#         root_data = objs_data[root_pid]
#         root_obj = self.obj_from_dict(root_data)
#         print("Root here:", root_obj.pid, root_obj.dpi)

#         # Step 2: Load and register non-root objects (e.g. TextElement, ImageElement)
#         for pid, obj_data in objs_data.items():
#             if pid == root_pid:
#                 continue
#             obj = self.obj_from_dict(obj_data)


#     def save_to_file(self, template, path: str):
#         data = self.dump_template(template)
...
#         # template = self.get(root_id)
#         self.load_template(data)
#         return template

#     def load_template(self, temp_data):
#         template = self.obj_from_dict(self,temp_data)
#         new_elements = []
#         for elem_data in template.elements:
#             element = self.obj_from_dict(elem_data)
#             template.insert_element(element)
#         return template

#     def dump_template(self, template):
#         dump_data = {}
#         element_data = []
#         for element in template.elements:
#             print(element)
#             element_data.append(self.obj_to_dict(element.pid))
#         dump_data = self.obj_to_dict(template.pid)
#         dump_data["elements"] = element_data
#         return dump_data
