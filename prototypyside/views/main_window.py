...
from prototypyside.views.font_toolbar import FontToolbar
...
class MainDesignerWindow(QMainWindow):
    ...
    def setup_property_editor(self):
        ...
        self.x_spin.valueChanged.connect(lambda val: self.update_selected_geometry(x=val))
        geometry_layout.addRow("X:", self.x_spin)

        self.y_spin = QDoubleSpinBox()
        self.y_spin.setRange(-9999.0, 9999.0)
        self.y_spin.setSingleStep(1.0)
        self.y_spin.valueChanged.connect(lambda val: self.update_selected_geometry(y=val))
        geometry_layout.addRow("Y:", self.y_spin)

        self.width_spin = QDoubleSpinBox()
        self.width_spin.setRange(MIN_ELEMENT_SIZE, 9999.0)
        self.width_spin.setSingleStep(1.0)
        self.width_spin.valueChanged.connect(lambda val: self.update_selected_geometry(width=val))
        geometry_layout.addRow("Width:", self.width_spin)

        self.height_spin = QDoubleSpinBox()
        self.height_spin.setRange(MIN_ELEMENT_SIZE, 9999.0)
        self.height_spin.setSingleStep(1.0)
        self.height_spin.valueChanged.connect(lambda val: self.update_selected_geometry(height=val))
        geometry_layout.addRow("Height:", self.height_spin)

        geometry_group.setLayout(geometry_layout)
        properties_layout.addWidget(geometry_group) # Add to the scrollable layout
        ...
        appearance_layout.addRow("Border Width:", self.border_width_spin)
        ...
        game_component_props_layout.addRow("Width (in):", self.game_component_width_spin)

        self.game_component_height_spin = QDoubleSpinBox()
        self.game_component_height_spin.setRange(1.0, 200.0)
        self.game_component_height_spin.setSingleStep(0.1)
        ...
        game_component_props_layout.addRow("Height (in):", self.game_component_height_spin)

        self.game_component_dpi_spin = QSpinBox()
        self.game_component_dpi_spin.setRange(72, 1200)
        self.game_component_dpi_spin.setSingleStep(1)
    ...
    def update_selected_geometry(self, x: Optional[float] = None, y: Optional[float] = None,
                                 width: Optional[float] = None, height: Optional[float] = None):
        element = self.get_selected_element()
        if element:
            current_rect = element.boundingRect()
            ...
            new_width = max(new_width, MIN_ELEMENT_SIZE)
            new_height = max(new_height, MIN_ELEMENT_SIZE)

            element.setPos(new_x, new_y)
            element.set_rect(QRectF(0, 0, new_width, new_height))
    ...
    def on_selection_changed(self):
        ...
        if selected_element:
            ...
            self.x_spin.setValue(selected_element.pos().x())
            self.y_spin.setValue(selected_element.pos().y())
            self.width_spin.setValue(selected_element.boundingRect().width())
            self.height_spin.setValue(selected_element.boundingRect().height())

            self.border_width_spin.setValue(selected_element._style.get('border_width', 0))
    ...
    def on_element_data_changed(self):
        ...
        if element and element == self._current_selected_element:
            ...
            self.x_spin.setValue(element.pos().x())
            self.y_spin.setValue(element.pos().y())
            self.width_spin.setValue(element.boundingRect().width())
            self.height_spin.setValue(element.boundingRect().height())

            self.border_width_spin.setValue(element._style.get('border_width', 0))
    ...
    def update_game_component_dimensions(self):
        new_width_in = self.game_component_width_spin.value()
    ...
    @Slot()
    def export_template_as_png(self): # Original GUI method
        if self.merged_templates:
            output_dir_str = QFileDialog.getExistingDirectory(self, "Select Output Directory for Merged PNGs")
            ...
            if export_count > 0:
                self.show_status_message(f"Successfully exported {export_count} merged templates to {output_dir.resolve()}", "success")
            else:
                self.show_status_message("PNG Export Failed: No merged templates were exported.", "error")
        else: # Export current template
            path, _ = QFileDialog.getSaveFileName(
    ...
    @Slot()
    def export_to_pdf(self): # Original GUI method
        path_str, _ = QFileDialog.getSaveFileName(
            self, "Export to PDF", "merged_templates.pdf", "PDF Files (*.pdf)")
        ...
        first_template = templates_to_export[0]
        ...
        try:
            for i, template_instance in enumerate(templates_to_export):
                if i > 0:
                    pdf_writer.newPage()
                ...
                if image:
                    ...
                    self.show_status_message(f"Warning: Could not render template {i+1} for PDF export.", "warning")

            painter.end()

            self.show_status_message(f"Successfully exported templates to {output_path}", "success")
        except Exception as e:
            QMessageBox.critical(self, "PDF Export Error", f"An error occurred during PDF export: {str(e)}")
            self.show_status_message(f"PDF Export Error: {str(e)}", "error")
            if painter.isActive():
                painter.end()


    # NEW: Export all merged templates as PNGs for CLI
    def export_png_cli(self, output_dir: Path):
        """
        Exports all merged templates (or current template if no merge) as PNGs
        to the specified directory in CLI (headless) mode.
        Messages are printed to console instead of dialogs.
        """
        self.set_cli_mode(True) # Ensure internal cli_mode flag is set

        templates_to_export = self.merged_templates if self.merged_templates else [self.current_template]

        if not templates_to_export:
            print("CLI Error: No templates to export as PNG.")
            return False
        ...
        for i, template_instance in enumerate(templates_to_export):
            image = self._render_template_to_image(template_instance)
            if image:
                if len(templates_to_export) > 1: # Naming convention for multiple merged files
                    filename = f"merged_output_{i+1}.png"
                else: # Naming convention for single current template export
                    filename = "current_template.png" # You can make this more dynamic if needed
    ...
    # NEW: Export all merged templates as PDF for CLI
    def export_pdf_cli(self, output_dir: Path):
        """
        Exports all merged templates (or current template if no merge) as a single PDF
        to the specified directory in CLI (headless) mode.
        Messages are printed to console instead of dialogs.
        """
        self.set_cli_mode(True) # Ensure internal cli_mode flag is set
        ...
        if not templates_to_export:
            print("CLI Error: No templates to export as PDF.")
            return False

        output_dir.mkdir(parents=True, exist_ok=True) # Ensure dir exists
        output_file_path = output_dir / "merged_output.pdf" # Fixed output name for PDF

        pdf_writer = QPdfWriter(str(output_file_path))
        first_template = templates_to_export[0]
        pdf_writer.setPageSizeMM(QSizeF(first_template.width_in * 25.4, first_template.height_in * 25.4))
        pdf_writer.setResolution(first_template.dpi) # Or a higher value like 600 or 1200 DPI
    ...
    def set_element_controls_enabled(self, enabled: bool):
        ...
        self.remove_element_btn.setEnabled(enabled) # Enable/Disable the new remove button
